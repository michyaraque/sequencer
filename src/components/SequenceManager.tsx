...
export default function SequenceManager({ sequences, onEdit, onDelete, onClose }: SequenceManagerProps) {
  ...
  return (
    ...
    >
      <div
        className="bg-white rounded-lg shadow-xl max-w-4xl w-full max-h-[95vh] sm:max-h-[90vh] flex flex-col"
        onClick={(e) => e.stopPropagation()}
      >
        <div className="p-3 sm:p-4 border-b border-neutral-200">
          <div className="flex items-center justify-between mb-3 sm:mb-4">
            <h2 className="text-lg sm:text-xl font-bold text-neutral-800">Sequence Manager</h2>
            <button
              onClick={onClose}
          ...
          <input
            ...
            className="w-full px-3 py-2 text-sm sm:text-base border border-neutral-300 rounded-md focus:outline-none focus:ring-2 focus:ring-neutral-500 bg-white"
          />
        </div>
          ...
          {sortedSequences.length === 0 ? (
            ...
            <div className="space-y-3">
              {sortedSequences.map((sequence) => (
                <div
                  key={sequence.id}
                  className="border border-neutral-200 rounded-lg p-3 sm:p-4 hover:border-neutral-400 transition-colors"
                >
                  <div className="flex flex-col sm:flex-row items-start justify-between gap-3 sm:gap-4">
                    <div className="flex-1 min-w-0 w-full">
                      <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                        <h3 className="font-bold text-neutral-800 text-base sm:text-lg">
                          {sequence.name}
                        </h3>
                      ...
                      <div className="flex flex-wrap gap-1.5 sm:gap-2 mb-2">
                        {sequence.nodes.slice(0, 5).map((node) => (
                          <span
                            key={node.id}
                            className="bg-neutral-100 text-neutral-700 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs font-mono"
                          >
                            {node.data.label || node.type}
                          </span>
                        ))}
                        {sequence.nodes.length > 5 && (
                          <span className="bg-neutral-100 text-neutral-500 px-1.5 sm:px-2 py-0.5 sm:py-1 rounded text-[10px] sm:text-xs">
                    ...
                    <div className="flex sm:flex-col gap-2 flex-shrink-0 w-full sm:w-auto">
                      <button
                        ...
                        className="flex-1 sm:flex-none px-3 py-1.5 bg-neutral-600 text-white rounded hover:bg-neutral-700 transition-colors text-xs sm:text-sm font-medium"
                      >
                        Edit
                      </button>
                      <button
                        onClick={() => handleDelete(sequence)}
                        className="flex-1 sm:flex-none px-3 py-1.5 bg-red-500 text-white rounded hover:bg-red-600 transition-colors text-xs sm:text-sm font-medium"
                      >
                        Delete
                      </button>
                    </div>
        ...
        <div className="p-3 sm:p-4 border-t border-neutral-200 bg-neutral-50">
          <div className="text-xs sm:text-sm text-neutral-600 flex flex-wrap gap-2">
            <span>
